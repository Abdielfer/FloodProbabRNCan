...
def load_show(stl_path: str, queue: Queue):  # Function to load and show the 3D object
    ...
    show(mesh, bg='black')  # Displaying the 3D object with a black background   , bg='black'
...
if __name__ == "__main__":  # Ensuring the script is being run directly and not imported
    ...
    if uploaded_file is not None:  # Checking if a file has been uploaded
        ...
        os.makedirs(f'./local/{unique_id}', exist_ok=True)  # Creating a new directory for the uploaded file
        file_path = f'./local/{unique_id}/file.stl'  # Setting the path for the uploaded file
        ...
        if st.button('Detener Streamlit'):
            shutdown_server()
